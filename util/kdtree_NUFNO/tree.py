import numpy as np
from util.kdtree_NUFNO.node import KDTreeNode
...
class KDTree:
    ...
    def solve(self, prepro=False, postpro=False):
        ...
        for node in self.nodes:
            # tot_nodes = tot_nodes + self.split([node], self.n_subdomains // max(self.group, 1), simple=True)
            tot_nodes = tot_nodes + self.split([node], self.n_subdomains // max(self.group, 1), simple=False)
    ...
    def split(self, nodes, n_subdomains, simple=False):
        while len(nodes) < n_subdomains:
            ...
            if simple:
                ...
                # son_a, son_b = nodes[ind[node_chosen]].overlap_split()
