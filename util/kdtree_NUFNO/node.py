...
class KDTreeNode:
    ...
    def overlap_split(self, overlap_factor=0.1):
        '''
        Split with overlapping regions
        '''
        m = len(self.points) >> 1
        borders_l, borders_r = self.borders[:], self.borders[:]
        split_val = np.median(np.array(self.points)[:, self.dim_chosen])
        borders_l[self.dim_chosen] = (
            borders_l[self.dim_chosen][0], split_val * (1 + overlap_factor))
        borders_r[self.dim_chosen] = (
            split_val * (1 - overlap_factor), borders_r[self.dim_chosen][1]
        )
        m = np.searchsorted(np.array(self.points)[:, self.dim_chosen], split_val)
    ...
    def simple_split(self):
        ...
        array_point = np.array(self.points)
        m = np.searchsorted(array_point[:, self.dim_chosen], split_val)
